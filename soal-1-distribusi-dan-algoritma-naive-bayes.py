...
Original file is located at
    https://colab.research.google.com/drive/1VM2m8yfmlrRUdfWwqSWEqwE35AU0-Bce
...
def generate_rand_uniform(lower_bound:float, upper_bound:float, num_samples:int):
  ...
  array = np.random.uniform(low=lower_bound, high=upper_bound, size=num_samples)
  # AKHIRI KODE DI SINI
...
def viz_uniform_teoretis(lower_bound, upper_bound, num_samples):
  ...
  variabel_uniform = generate_rand_uniform(lower_bound, upper_bound, num_samples)
  ...
  plt.hist(variabel_uniform, bins=30, density=True, color='lightgrey', edgecolor='black')
...
def generate_gaussian(miu, sigma, num_samples):
  ...
  # MULAI KODE DI SINI
  y = generate_rand_uniform(0, 1, num_samples)
  # Petunjuk:
  ...
  array = uniform_inverse_cdf(y, miu, sigma)
  # AKHIRI KODE DI SINI
...
def viz_distribusi_gaussian(first_gaussian, second_gaussian, third_gaussian):
  ...
  ax.hist(first_gaussian, bins=32, alpha=0.5, label='First Gaussian')
  ax.hist(second_gaussian, bins=32, alpha=0.5, label='Second Gaussian')
  ax.hist(third_gaussian, bins=32, alpha=0.5, label='Third Gaussian')
...
def generate_binomial(n_trials, success_prob, num_samples):
  ...
  array = binomial_inverse_cdf(y, n_trials, success_prob)
  # AKHIRI KODE DI SINI
...
def viz_distribusi_binomial(first_binomial, second_binomial, third_binomial):
  ...
  ax.hist(first_binomial, alpha=0.5, label='First Binomial')
  ax.hist(second_binomial, alpha=0.5, label='Second Binomial')
  ax.hist(third_binomial, alpha=0.5, label='Third Binomial')
  # AKHIRI KODE DI SINI
...
def training_params_estimation(df, features):
    ...
    for breed in df['breed'].unique():
        ...
        probs_dict[breed] = len(df_breed) / total_rows
        ...
        for feature in features:
            if feature in ["wingspan_cm", "weight_g"]:
                ...
                p = df_breed[feature].mean() / n_trials
